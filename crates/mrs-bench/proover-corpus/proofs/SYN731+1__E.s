...
%------------------------------------------------------------------------------
% File     : E---3.3.0
% Problem  : SYN731+1 : TPTP v9.2.0. Released v2.5.0.
% Transfm  : none
% Format   : tptp:raw
...
%            Number of variables   :   16 (   1 sgn   6   !;   6   ?)
...
fof(x2150,conjecture,
    ? [X1] :
      ( ! [X2] :
        ? [X3] : p(X1,X2,X3)
     => ? [X4] : p(X4,X4,X1) ),
    file('/export/starexec/sandbox/benchmark/theBenchmark.p',x2150) ).
...
fof(c_0_1,negated_conjecture,
    ~ ? [X1] :
        ( ! [X2] :
          ? [X3] : p(X1,X2,X3)
       => ? [X4] : p(X4,X4,X1) ),
    inference(assume_negation,[status(cth)],[x2150]) ).

fof(c_0_2,negated_conjecture,
    ! [X5,X6,X8,X9] :
      ( p(X5,X6,esk1_2(X5,X6))
      & ~ p(X9,X9,X8) ),
...
% 0.03/0.12  % Problem    : SYN731+1 : TPTP v9.2.0. Released v2.5.0.
...
% 0.12/0.33  % DateTime   : Fri Sep 26 14:36:38 EDT 2025
% 0.12/0.33  % CPUTime    : 
...
% 0.21/0.48  # new_bool_3 with pid 15577 completed with status 0
...
% 0.21/0.48  # Search class: FUUNF-FFSF22-SFFFFFNN
...
% 0.21/0.48  # SAT001_MinMin_p005000_rr_RG with pid 15583 completed with status 0
...
% 0.21/0.48  # Initial clauses                      : 2
% 0.21/0.48  # Removed in clause preprocessing      : 0
% 0.21/0.48  # Initial clauses in saturation        : 2
% 0.21/0.48  # Processed clauses                    : 4
% 0.21/0.48  # ...of these trivial                  : 0
% 0.21/0.48  # ...subsumed                          : 0
% 0.21/0.48  # ...remaining for further processing  : 4
...
% 0.21/0.48  # Current number of archived clauses   : 2
...
% 0.21/0.48  # Termbank termtop insertions          : 129
% 0.21/0.48  # Search garbage collected termcells   : 47
% 0.21/0.48  
% 0.21/0.48  # -------------------------------------------------
% 0.21/0.48  # User time                : 0.003 s
% 0.21/0.48  # System time              : 0.001 s
% 0.21/0.48  # Total time               : 0.004 s
% 0.21/0.48  # Maximum resident set size: 1592 pages
% 0.21/0.48  
% 0.21/0.48  # -------------------------------------------------
% 0.21/0.48  # User time                : 0.003 s
% 0.21/0.48  # System time              : 0.004 s
% 0.21/0.48  # Total time               : 0.007 s
% 0.21/0.48  # Maximum resident set size: 1688 pages
% 0.21/0.48  % E exiting
% 0.21/0.48  % E exiting
%------------------------------------------------------------------------------
