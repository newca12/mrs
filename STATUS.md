...
- **Redundancy Elimination**:
  ...
  - **Subsumption Resolution**: Contextual literal cutting using the Feature Vector Index.
...
- **Goal-Directed UEQ Heuristics**: Integrated a distance-to-conjecture tracker directly into the `Clause` structure to drastically penalize pure-axiom generated clauses in weight selection.
...
2. **InstGen / EPR Handlers**: The Effectively Propositional fragment needs an instantiation-based engine, as superposition is often overkill.
3. **AC-Unification**: Associativity and commutativity axioms currently cause permutation explosions in the search space.
4. **Machine Learning**: Deepire/ENIGMA clause selection using Graph Neural Networks.

## Next Steps

1. Upgrade `DTree` to prevent false-positive unifications.
2. Build an InstGen subsystem for the EPR division.