use std::sync::Arc;
use mrs_core::clause::{ClauseIdGen, ClauseSource, Clause};
use mrs_core::{Atom, Literal, SymbolTable, Term};
use mrs_search::state::SearchState;
use mrs_search::{LiteralSelection, SearchConfig, SearchResult, SelectionStrategy, TermOrdering};
...
fn main() {
    ...
    let clauses = vec![
        input_clause(&mut id_gen, vec![Literal::pos(Atom::pred(f_sym, vec![Term::constant(sk1)]))], "ax1_0"),
        input_clause(&mut id_gen, vec![Literal::neg(Atom::pred(g_sym, vec![Term::constant(sk1)]))], "ax1_1"),
        input_clause(&mut id_gen, vec![
        ...
        input_clause(&mut id_gen, vec![Literal::pos(Atom::pred(j_sym, vec![Term::constant(sk2)]))], "goal_0"),
        input_clause(&mut id_gen, vec![Literal::pos(Atom::pred(i_sym, vec![Term::constant(sk2)]))], "goal_1"),
    ];
