...
> Status: Dated challenge report. Problem outcomes are local observations, not
> official results or current guarantees.
...
### Solver Limits: Bounded-Counter EPR Encodings

#### SWV/SWV418-1.*, SWV421-1.*, & SWV422-1.*
* **SZS Status:** Unsatisfiable
* **Observed status:** `Timeout` in focused local runs on the in-tree
  `casc-30/EPU` instances, including `SWV418-1.300.p`, `SWV421-1.200.p`, and
  `SWV422-1.300.p`.
* **Technical Reason:** These software-verification problems encode bounded
  counter or transition-system behavior as function-free CNF. The clauses are
  overwhelmingly Horn and ground, but the encoding includes a large
  disjunction enumerating possible transitions (for example, a clause with
  hundreds of alternatives). InstGen classifies them as pure relational EPR,
  but its SAT-guided instance generation and the ordinary given-clause search
  did not derive a refutation within the focused time limits. The 750 ms
  InstGen prepass budget is now enforced inside the CaDiCaL solve and during
  instance generation; this avoids overrunning that prepass budget but has not
  solved the counter examples.
* **Evidence:** The representative problem files are tracked under
  `crates/mrs-bench/problems/casc-30/EPU/`. Focused runs used the in-tree
  release binary with `--workers 1 --strategy 1 --schedule casc_epu`; all three
  named instances timed out at 20–30 seconds. These are diagnostic local runs,
  not official CASC scores or claims that the family is unsolvable.
