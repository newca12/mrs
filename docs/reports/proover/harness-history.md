...
> Status: Historical harness report. Current commands are in
> [`guides/proover.md`](../../guides/proover.md).
...
This note documents the **test/regression infrastructure** for `mrs-proover`
 ...
 deterministic corpus works. The verifier's scoring roadmap is historical and
 is retained in [`history/todo-proover.md`](../../history/todo-proover.md).
...
- PyRes: mrs-proover verifies **160/170**, Nörgler 155/170. The earlier coverage
  ...
  (`(A∧B)∧(C∧D)` → `A∧(B∧(C∧D))`); the structural matcher is not yet AC-aware, so
   it safely declines (0 pts) rather than risk a −1. (Tracked in the archived
   [`history/todo-proover.md`](../../history/todo-proover.md).)
