...
The Nix development shell appends `-C target-cpu=native` for local builds. This
allows the compiler to use the instruction set available on the build host. It
is not a portable release target and there is no repository `.cargo/config.toml`
that should be copied into a deployment package.

For a portable release, choose the target explicitly:
...
```text
/path/to/tptp/Systems/mrs---0.2.3/
├── mrs
└── run_mrs
```
...
```bash
/path/to/tptp/Systems/mrs---0.2.3/run_mrs /path/to/problem.p 60
```
